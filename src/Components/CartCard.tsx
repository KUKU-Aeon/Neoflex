...
export const CartCard = () => {
    ...
    const Cart: Device[] = useSelector((state: Device[]) => state);
    ...
    const handleRemoveFromCart = (elem: Device) => {
        dispatch(actions.removeFromCart(elem));
    }

    const handleRemoveFromCartAll = (elem: Device) => {
        dispatch(actions.removeFromCartAll(elem));
    }

    const Price = Cart.reduce((acc, el: Device) => {
            ...
            <form>
                <div className="card-number">
                    ...
                    <input required type="text" id="card-number" name="card-number" inputMode="numeric"
                           autoComplete="cc-number"  placeholder="Введите номер карты" pattern="^(?:4[0-9]{12}(?:[0-9]{3})?|[25][1-7][0-9]{14}|6(?:011|5[0-9][0-9])[0-9]{12}|3[47][0-9]{13}|3(?:0[0-5]|[68][0-9])[0-9]{11}|(?:2131|1800|35\d{3})\d{11})$"/>
                </div>
                <div>
